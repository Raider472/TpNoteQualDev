...
export class Menu {
    ...
    public static choixCompétition(nom: string, debut:Date, fin:Date): Decathlon | Heptathlon | null {
        let resultat = readlineSync.questionFloat("Quelle compétition voulez-vous gérer (1=décathlon ou 2=heptathlon ; 0 pour quitter): ")
        for (let i = false; i != true; ) {
            if (resultat === 1) {
                return new Decathlon(nom, debut, fin)
            }
            else if (resultat === 2) {
                return new Heptathlon(nom, debut, fin)
            }
            else if (resultat === 0) {
                i = true
            }
            else {
                resultat = readlineSync.questionFloat("Quelle compétition voulez-vous gérer (1=décathlon ou 2=heptathlon ; 0 pour quitter): ")
            }
        }
        return null
    }
